...
class TTSConfig:
    ...
    def __init__(
        self,
        common: TTSCommonConfig = None,
        provider_config: Dict[str, Any] = None,
    ):
        self.common = common or TTSCommonConfig()
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "TTSConfig":
        ...
        if not isinstance(data, dict):
            raise ValueError("TTS config must be a JSON object")

        common_data = cls._merge_grouped_section(data.get("common", {}), "tts.common")
        common = TTSCommonConfig(
            provider=data.get("provider", "huoshan"),
            sample_rate=common_data.get("sample_rate", 24000),
        )

        provider_name = common.provider.lower()
        providers_data = data.get("providers", {})
        if not isinstance(providers_data, dict):
            raise ValueError("TTS config must contain providers object")

        provider_section = providers_data.get(provider_name)
        if not isinstance(provider_section, dict):
            raise ValueError(f"TTS config missing providers.{provider_name}")

        provider_data = cls._merge_grouped_section(
            provider_section,
            f"tts.providers.{provider_name}",
        )
    ...
    @classmethod
    def from_file(cls, config_path: str = None) -> "TTSConfig":
        """从 JSON 配置文件加载配置
    ...
    def validate(self) -> Tuple[bool, str]:
        ...
        for field in required_fields:
            ...
            if provider == "deepgram" and field == "api_key" and (not value or str(value).strip() == ""):
                from src.config.env import settings

                value = getattr(settings, "deepgram_api_key", "").strip()
