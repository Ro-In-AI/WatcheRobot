...
class ASRConfig:
    ...
    def __init__(
        self,
        common: ASRCommonConfig = None,
        provider_config: Dict[str, Any] = None,
    ):
        self.common = common or ASRCommonConfig()
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ASRConfig":
        ...
        common_data = cls._merge_grouped_section(data.get("common", {}), "asr.common")
        common = ASRCommonConfig(
            provider=data.get("provider", "aliyun"),
            sample_rate=common_data.get("sample_rate", 16000),
            channels=common_data.get("channels", 1),
        )

        provider_name = common.provider.lower()
        providers_data = data.get("providers", {})
        if not isinstance(providers_data, dict):
            raise ValueError("ASR config must contain providers object")

        provider_section = providers_data.get(provider_name)
        if not isinstance(provider_section, dict):
            raise ValueError(f"ASR config missing providers.{provider_name}")

        provider_data = cls._merge_grouped_section(
            provider_section,
            f"asr.providers.{provider_name}",
        )
    ...
    @classmethod
    def from_file(cls, config_path: str = None) -> "ASRConfig":
        """从 JSON 配置文件加载配置
    ...
    def validate(self) -> Tuple[bool, str]:
        ...
        if provider == "aliyun":
            appkey = str(self.provider_config.get("appkey", "")).strip()
            token = str(self.provider_config.get("token", "")).strip()
            ak_id = str(self.provider_config.get("ak_id", "")).strip()
            ak_secret = str(self.provider_config.get("ak_secret", "")).strip()

            if not appkey:
                return False, "配置验证失败: aliyun 提供商缺少必填字段: appkey"

            has_token_auth = bool(token)
            has_aksk_auth = bool(ak_id and ak_secret)
            if not has_token_auth and not has_aksk_auth:
                return False, "配置验证失败: aliyun 提供商需要 token，或同时提供 ak_id 与 ak_secret"
